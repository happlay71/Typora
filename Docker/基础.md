...
## 安装教程来源：[黑马飞书文档](https://b11et3un53m.feishu.cn/wiki/Rfocw7ctXij2RBkShcucLZbrn2d)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/17195580433541.png)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/17195582047564.png)

以及对应的初始化SQL脚本和配置文件：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/17195583247777.png)

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/17195583247788.png)

其中，hm.cnf主要是配置了MySQL的默认编码，改为utf8mb4；而hmall.sql则是后面我们要用到的黑马商城项目的初始化SQL脚本。

我们直接将整个mysql目录上传至虚拟机的`/root`目录下：![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955834767413.png)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955838060016.png)2.3.2.Dockerfile
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955843374919.png)

首先，我们将课前资料提供的`docker-demo.jar`包以及`Dockerfile`拷贝到虚拟机的`/root/demo`目录：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955843374920.png)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955844591025.png)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371628.png)

我们要部署的就是其中的`hm-service`，其中的配置文件采用了多环境的方式：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371729.png)

其中的`application-dev.yaml`是部署到开发环境的配置，`application-local.yaml`是本地运行时的配置。

查看application.yaml，你会发现其中的JDBC地址并未写死，而是读取变量：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371730.png)

这两个变量在`application-dev.yaml`和`application-local.yaml`中并不相同：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371731.png)

在dev开发环境（也就是Docker部署时）采用了mysql作为地址，刚好是我们的mysql容器名，只要两者在一个网络，就一定能互相访问。

我们将项目打包：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371732.png)

结果：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371733.png)

将`hm-service`目录下的`Dockerfile`和`hm-service/target`目录下的`hm-service.jar`一起上传到虚拟机的`root`目录：

![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955849371734.png)部署项目：
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955850847449.png)
...
![img](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/171955850847450.png)然后创建nginx容器并完成两个挂载：
